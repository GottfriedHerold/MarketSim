...
class MyMarket(Market):
    ...
    def reveal_payments(self, reveal_side: list[Cluster]) -> float:
        reveal = sum(self.standing_bids[c].willing_to_pay for c in reveal_side if self.standing_bids[c] is not None)
        return reveal

  
    def not_reveal_payments(self, miss_side: list[Cluster]) -> float:
        not_reveal = sum(self.standing_bids[c].willing_to_pay for c in miss_side if self.standing_bids[c] is not None)
        return not_reveal
